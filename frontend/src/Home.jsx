...
// MAKE SURE THIS MATCHES YOUR CSS FILE NAME
            ...
            <main className="relative z-10 max-w-6xl mx-auto px-6 pt-24 pb-20 space-y-32">
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        <div className="space-y-6">
                            ...
                            <div className="flex flex-wrap gap-4 pt-4">
                                <button onClick={() => scrollTo('contact')} className="px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium transition-all shadow-lg shadow-teal-500/20 flex items-center space-x-2">
                                    <Mail size={18} />
                            ...
                            <div className="relative bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-8 shadow-2xl">
                                <div className="space-y-6">
                                    <div className="flex items-start space-x-4">
                                        <div className="p-3 bg-teal-500/10 rounded-lg text-teal-400"><MapPin size={24} /></div>
                                        <div><h3 className="text-white font-medium">Location</h3><p className="text-slate-400">Veldhoven, Netherlands</p></div>
                                    </div>
                                    <div className="flex items-start space-x-4">
                                        <div className="p-3 bg-cyan-500/10 rounded-lg text-cyan-400"><Phone size={24} /></div>
                                        <div><h3 className="text-white font-medium">Phone</h3><p className="text-slate-400">06 12443024</p></div>
                                    </div>
                                    <div className="flex items-start space-x-4">
                                        <div className="p-3 bg-sky-500/10 rounded-lg text-sky-400"><Mail size={24} /></div>
                                        <div><h3 className="text-white font-medium">Email</h3><p className="text-slate-400">jmol577@hotmail.com</p></div>
                                    </div>
                                    <div className="flex items-start space-x-4">
                                        <div className="p-3 bg-blue-500/10 rounded-lg text-blue-400"><User size={24} /></div>
                                        <div><h3 className="text-white font-medium">Personal</h3><p className="text-slate-400">Nationality: Dutch</p></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <section id="skills" className="scroll-mt-24">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <SkillCard title="HTML" level="Skillful" icon={<Globe className="text-orange-500" />} progress={90} />
                        <SkillCard title="CSS" level="Skillful" icon={<Code2 className="text-blue-500" />} progress={85} />
                        <SkillCard title="JavaScript" level="Beginner" icon={<Code2 className="text-yellow-400" />} progress={40} />
                        <SkillCard title="WordPress" level="Experienced" icon={<Globe className="text-blue-400" />} progress={75} />
                        <SkillCard title="C#" level="Beginner" icon={<Code2 className="text-purple-500" />} progress={30} />
                    </div>
                ...
                <div className="grid lg:grid-cols-2 gap-12">
                    <section id="experience" className="scroll-mt-24">
                        <div className="mb-8">
                            <h2 className="text-3xl font-bold text-white mb-4 flex items-center space-x-3"><Briefcase className="text-cyan-400" /><span>History</span></h2>
                        </div>
                        <div className="relative border-l border-slate-800 ml-3 space-y-8">
                            <TimelineItem date="Jan 2023 — Present" title="Evening Operations" subtitle="Seabourne Logistics, Eindhoven" description="Managing logistics and operational tasks efficiently during evening shifts." current />
                        </div>
                    </section>

                    <section id="education" className="scroll-mt-24">
                        <div className="mb-8">
                            <h2 className="text-3xl font-bold text-white mb-4 flex items-center space-x-3"><GraduationCap className="text-teal-400" /><span>Education</span></h2>
                        </div>
                        <div className="relative border-l border-slate-800 ml-3 space-y-8">
                            <TimelineItem date="Aug 2022 — Present" title="SintLucas, Eindhoven" subtitle="Web Development" description="Currently studying at SintLucas." current />
                            <TimelineItem date="Aug 2018 — May 2022" title="VMBO-GT, Yuverta" subtitle="Secondary Education" description="Completed secondary education." />
                        </div>
                    </section>
                </div>
                ...
                <footer id="contact" className="border-t border-slate-800 pt-12 text-center">
                    ...
                    <a href="mailto:jmol577@hotmail.com" className="inline-flex items-center space-x-2 text-teal-400 hover:text-teal-300 font-medium text-lg transition-colors">
                        <Mail /><span>jmol577@hotmail.com</span>
                    </a>
                    <div className="mt-12 text-slate-600 text-sm pb-8">© {new Date().getFullYear()} Jari Mol. All rights reserved.</div>
                </footer>
            </main>
        </div>
    );
...
const SkillCard = ({ title, level, icon, progress }) => (
    <div className="group bg-slate-900/50 hover:bg-slate-800/60 border border-slate-800 hover:border-teal-500/30 transition-all duration-300 p-6 rounded-xl">
        <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider bg-slate-950 px-2 py-1 rounded">{level}</span>
        </div>
        <h3 className="text-lg font-bold text-white mb-2">{title}</h3>
        <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full transition-all duration-1000 ease-out" style={{ width: `${progress}%` }}></div>
        </div>
    </div>
);
